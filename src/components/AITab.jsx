...
Please answer the user's question about this code in a clear and helpful way using Markdown formatting.`
  ...
  return (
    <div className="flex flex-col flex-1 gap-4 min-h-0">
      {/* Chat messages area */}
      <div className="bg-white dark:bg-[#111216] rounded-lg p-3 text-xs text-gray-700 dark:text-gray-300 space-y-3 flex-1 overflow-y-auto overflow-x-hidden min-h-0 max-h-[400px] border border-gray-200 dark:border-[#3e3e42]">
        {messages.length === 0 ? (
          <>
              ...
              <div
                ...
                className={`p-2 rounded-lg ${
                  msg.role === 'user'
                    ...
                    : 'bg-gray-100 dark:bg-[#111216] text-gray-700 dark:text-gray-300 mr-4 border border-gray-200 dark:border-[#3e3e42]'
                }`}
              >
                    ...
                    <ReactMarkdown
                      components={{
                        ...
                        code: ({ children, className }) => {
                          ...
                          return isInline ? (
                            <code className="bg-gray-200 dark:bg-[#1a1b1f] px-1 py-0.5 rounded text-[0.7rem]">{children}</code>
                          ) : (
                            <code className="block bg-gray-200 dark:bg-[#1a1b1f] p-2 rounded text-[0.7rem] overflow-x-auto">{children}</code>
                          )
                        },
                        pre: ({ children }) => <pre className="bg-gray-200 dark:bg-[#1a1b1f] p-2 rounded text-[0.7rem] overflow-x-auto mb-2">{children}</pre>,
                        ...
                        blockquote: ({ children }) => <blockquote className="border-l-4 border-gray-300 dark:border-[#3e3e42] pl-2 italic mb-2">{children}</blockquote>,
                      }}
                    >
                      {msg.content}
                    </ReactMarkdown>
      ...
      {/* Input area */}
      <div className="border border-gray-200 dark:border-[#3e3e42] rounded-lg p-2 flex flex-col gap-2 bg-white dark:bg-[#111216]">
        <textarea
          value={question}
