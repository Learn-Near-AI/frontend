...
function TourButton({ onStartTour }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {showTooltip && (
        <div className="absolute bottom-full right-0 mb-3 w-64 bg-[#1a1b1f] border border-[#3e3e42] rounded-lg shadow-xl p-4 animate-in slide-in-from-bottom-2">
          <button
          ...
          <div className="absolute -bottom-2 right-6 w-4 h-4 bg-[#1a1b1f] border-r border-b border-[#3e3e42] transform rotate-45" />
        </div>
      )}
      ...
      >
        ...
        {/* Hover tooltip */}
        <span className="absolute bottom-full right-0 mb-2 whitespace-nowrap bg-[#1a1b1f] text-white text-xs px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none border border-[#3e3e42]">
          Take a guided tour
          <span className="absolute -bottom-1 right-4 w-2 h-2 bg-[#1a1b1f] border-r border-b border-[#3e3e42] transform rotate-45" />
        </span>
      </button>
    </div>
  )
